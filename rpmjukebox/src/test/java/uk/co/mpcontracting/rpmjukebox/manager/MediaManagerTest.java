...
public class MediaManagerTest extends AbstractTest {
	...
	public void shouldSetVolumePercent() {
		...
		assertThat("Volume should be 0.5", (double)ReflectionTestUtils.getField(spyMediaManager, "volume"), equalTo(0.5d));
		assertThat("Muted should be false", (boolean)ReflectionTestUtils.getField(spyMediaManager, "muted"), equalTo(false));
	...
	public void shouldSetVolumePercentZero() {
		...
		assertThat("Muted should be true", (boolean)ReflectionTestUtils.getField(spyMediaManager, "muted"), equalTo(true));
		verify(mockMediaPlayer, times(1)).setVolume(anyDouble());
		verify(getMockEventManager(), times(1)).fireEvent(Event.MUTE_UPDATED);
	}
