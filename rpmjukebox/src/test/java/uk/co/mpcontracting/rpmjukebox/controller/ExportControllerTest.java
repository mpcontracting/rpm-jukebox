...
public class ExportControllerTest extends AbstractTest {
    ...
    public void shouldBindPlaylists() throws Exception {
        ...
        ObservableList<PlaylistTableModel> observablePlaylists = (ObservableList<PlaylistTableModel>)ReflectionTestUtils.getField(spyExportController, "observablePlaylists");
    ...
    public void shouldAddPlaylistToExport() throws Exception {
        ...
        @SuppressWarnings("unchecked")
        PlaylistTableModel playlistTableModel1 = ((ObservableList<PlaylistTableModel>)ReflectionTestUtils.getField(spyExportController, "observablePlaylists")).get(0);
        @SuppressWarnings("unchecked")
        PlaylistTableModel playlistTableModel2 = ((ObservableList<PlaylistTableModel>)ReflectionTestUtils.getField(spyExportController, "observablePlaylists")).get(1);
        
        playlistTableModel1.getSelected().set(true);
        playlistTableModel2.getSelected().set(true);
        
        @SuppressWarnings("unchecked")
        Set<Integer> playlistsToExport = (Set<Integer>)ReflectionTestUtils.getField(spyExportController, "playlistsToExport");
    ...
    public void shouldClickOkButtonWithExceptionThrownFromFileWriter() throws Exception {
        ...
        FileWriter mockFileWriter = mock(FileWriter.class);
        doThrow(new RuntimeException("ExportControllerTest.shouldClickOkButtonWithExceptionThrownFromFileWriter()")).when(mockFileWriter).write(anyString());
        doReturn(mockFileWriter).when(spyExportController).constructFileWriter(any());
